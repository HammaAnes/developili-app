...
import 'Form_client_qst7.dart'; // Importez la page que vous souhaitez afficher après avoir cliqué sur un des premiers boutons
...
class _My_6th_question_State extends State<My_6th_question>
  ...
  Future<void> _submitAnswer(String answer) async {
    ...
    try {
      ...
      final result = await APIService.submitAnswer(id, 6, answer, 'handle_questions', 'null'); // Example: client_id = 1, question_id = 1
      if (result["success"]) {
  ...
  try {
    final storage = FlutterSecureStorage();
      String? user_id = await storage.read(key: "user_id");
      int? id = getUserId(user_id);
    final result = await APIService.deleteAnswer(id , 5, 'handle_questions'); // Replace with the actual client ID and question ID
    if (result["success"]) {
            ...
            Positioned(
              ...
              child: Center(
                child: Padding(
                  ...
                  child: Text(
                    "How do you prioritize between budget, quality, and timeline ?",
                    style: TextStyle(
                      fontSize: 28,
                ...
                children: List.generate(totalPages, (index) {
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: index < 6 // Colore nombre de cercles
