...
class _My_3rd_question_State extends State<My_3rd_question>
  ...
  Future<void> _submitAnswer(String answer) async {
    ...
    try {
      ...
      final result = await APIService.submitAnswer(id, 3, answer, 'handle_questions', 'null'); // Example: client_id = 1, question_id = 1
      if (result["success"]) {
  ...
  try {
    final storage = FlutterSecureStorage();
      String? user_id = await storage.read(key: "user_id");
      int? id = getUserId(user_id);
    final result = await APIService.deleteAnswer(id, 2, 'handle_questions'); // Replace with the actual client ID and question ID
    if (result["success"]) {
            ...
            Positioned(
              ...
              child: Center(
                child: Padding(
                  ...
                  child: Text(
                    "What is your level of knowledge in development ?",
                    style: TextStyle(
                      fontSize: 28,
                ...
                children: List.generate(totalPages, (index) {
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: index < 3 // Colore nombre de cercles
